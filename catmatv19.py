...
layout_explorador = [
    ...
    [sg.Frame('3. Ações', [[
        sg.Button('Apenas Buscar CATMATs', key='-BUSCAR_CATMATS-'),
...
def buscar_catmats_thread(pdms_selecionados, window, acao_apos_busca):
    df_catmats = buscar_catmats_por_pdm(pdms_selecionados, URL_BASE, TIMEOUT, window)
    window.write_event_value('-THREAD_CONCLUIDA-', (df_catmats, acao_apos_busca))
...
while True:
    ...
    if event == '-THREAD_CONCLUIDA-':
        df_catmats, acao_apos_busca = values[event]
        ...
        if df_catmats is not None and 'codigoItem' in df_catmats.columns:
            ...
            if acao_apos_busca == 'extrair':
                window['-TAB_EXTRACAO-'].select()
                window["-OUTPUT-"].print(f"🔎 {num_catmats} CATMATs descobertos. Extração iniciada automaticamente.", text_color='lightblue')
                iniciar_processo_extracao(lista_catmats_descobertos)
            else: 
    ...
    if processing:
        try:
            ...
            if not saved_parts:
                ...
                # ***** LÓGICA DE RELATÓRIO OTIMIZADA E MAIS INTELIGENTE *****
                wb_rel = Workbook()
