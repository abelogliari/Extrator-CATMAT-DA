...
def pagina_corrompida(csv_text: str, INDICE_COLUNA_T: int, INDICE_COLUNA_AI: int) -> Tuple[bool, Optional[pd.DataFrame], str]:
    # (Esta função permanece exatamente a mesma do código original)
...
# Ponto 4: Tema mais neutro e profissional
sg.theme('SystemDefaultForReal')

# Ponto 1: Divisão em seções claras usando Frames
layout_config = [
...
# Ponto 2: Contadores em destaque para feedback visual rápido
layout_stats = [
    ...
    [sg.Text("Códigos sem Dados:", size=(20,1)), sg.Text("0", key="-CONT_VAZIOS-", font=("Helvetica", 10, "bold"), text_color="#FF6347")], # Tomato Red
]
...
while True:
    ...
    if event == "-START-" and not processing:
        ...
        window["-OUTPUT-"].update(""); window["-PROGRESS-"].update(0)
        
        # Resetar contadores da UI
    ...
    if event == "-CANCEL-" and processing:
        ...
        codes_iterator = None
        window["-PROGRESS-"].update(0); window["-START-"].update(disabled=False); window["-CANCEL-"].update(disabled=True)
        ...
        try:
            ...
            try:
                ...
                if dfp is None or dfp.empty or csv_text is None:
                    window["-OUTPUT-"].print(f"ℹ️ Código {codigo}: Nenhum registro encontrado na primeira página.", text_color='lightblue'); codigos_vazios_count += 1; window["-CONT_VAZIOS-"].update(codigos_vazios_count)
                    raise StopIteration
                
                m_reg = re.search(r"totalRegistros\s*:\s*(\d+)", csv_text, re.IGNORECASE)
                registros_esperados_por_codigo[codigo] = int(m_reg.group(1)) if m_reg else 0
                if registros_esperados_por_codigo[codigo] == 0:
                ...
                while True:
                    ...
                    if is_corrupt:
                        ...
                        if df_corrigido is not None and not df_corrigido.empty:
                            window["-OUTPUT-"].print(f"⚠️ Cód {codigo}, Pág {pagina_atual}: Corrompida, mas foi corrigida e integrada.", text_color='orange'); df_final_pagina = df_corrigido
                            paginas_corrigidas_count += 1; window["-CONT_CORRIGIDAS-"].update(paginas_corrigidas_count)
                        else: window["-OUTPUT-"].print(f"❌ Cód {codigo}, Pág {pagina_atual}: Corrompida, não foi possível reconstruir.", text_color='#FF6347')
                    else:
            ...
            window['-PROGRESS-'].update(int((idx / total_codigos) * 950))

        except StopIteration:
            processing = False
            window["-STATUS-"].update("Status: Gerando relatório final..."); window["-PROGRESS-"].update(980)
            ...
            for c in codigos_itens:
                registros_baixados = int(registros_baixados_por_codigo.get(c, 0))
                registros_esperados_api = int(registros_esperados_por_codigo.get(c, 0))
                paginas = paginas_corrompidas_por_codigo.get(c, [])
                status = "OK" if registros_baixados >= registros_esperados_api else f"Inconsistência (baixados={registros_baixados} / esperados={registros_esperados_api})"
                ws_rel.append([c, registros_esperados_api, registros_baixados, ", ".join(map(str, paginas)), status])
...
-------------------------------------------
            ...
            window["-PROGRESS-"].update(1000); window["-STATUS-"].update("Status: Concluído!"); window["-START-"].update(disabled=False); window["-CANCEL-"].update(disabled=True); window["-SAVE_LOG-"].update(disabled=False)
