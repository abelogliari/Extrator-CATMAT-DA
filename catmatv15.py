...
def pagina_corrompida(df: pd.DataFrame, INDICE_COLUNA_T: int, INDICE_COLUNA_AI: int, csv_text: str = None) -> bool:
    """
    Detecta e tenta corrigir CSVs com linhas quebradas/aspas desalinhadas.
    Retorna True somente se conseguiu reconstruir e aplicar o DataFrame corrigido em `df`.
    """
    if not csv_text:
        return False

    linhas_raw = csv_text.splitlines()
    # remove linhas vazias e linhas de controle
    clean_lines = [
        ln for ln in linhas_raw
        if ln.strip() != "" and not ln.strip().lower().startswith(("totalregistros:", "totalpaginas:"))
    ]
    if not clean_lines:
        return False

    # ---- util: conta campos (sep ;) ignorando ; que estão dentro de aspas ----
    def count_fields_unquoted(line: str) -> int:
        ...
        while i < len(line):
            ch = line[i]
            # double-quote handling with escaped "" sequence
            if ch == '"':
                if not in_sq:
                ...
                continue
            if ch in ('“', '”'):
                # treat curly quotes as toggles for double-quote state
                if not in_sq:
                    in_dq = not in_dq
                i += 1
                continue
    ...
    # ---- 1) Tenta ler com pandas e detecta inconsistência por contagem de campos ----
    header_count = None
    try:
        df_try = pd.read_csv(
            StringIO("\n".join(clean_lines)),
        ...
        header_count = len(df_try.columns)
        # checa se todas as linhas têm o mesmo número de campos (segundo nosso parser)
        mismatches = sum(1 for ln in clean_lines if count_fields_unquoted(ln) != header_count)
        if mismatches == 0:
            # CSV parece consistente -> não é corrompido
            return False
    except Exception:
        # falhou ao ler diretamente -> vamos tentar reconstruir
        pass

    # ---- 2) Reconstrução: juntar linhas conforme necessidade ----
    reconstructed = []
    i = 0
    n = len(clean_lines)
    ...
    def is_balanced_quotes(s: str) -> bool:
        ...
        while j < len(s):
            ...
            if ch == '"':
                if not in_sq:
                    if in_dq and (j + 1) < len(s) and s[j + 1] == '"':
                        j += 2
                        continue
                    in_dq = not in_dq
                j += 1
            ...
            if ch == "'":
                if not in_dq:
                    if in_sq and (j + 1) < len(s) and s[j + 1] == "'":
                        j += 2
                        continue
                    in_sq = not in_sq
                j += 1
                continue
            j += 1
        return not (in_dq or in_sq)

    while i < n:
        buf = clean_lines[i]
        i += 1

        # se a linha atual começar com ';' e já houver registro anterior,
        # é continuação do anterior (ex.: seu caso)
        if buf.lstrip().startswith(';') and reconstructed:
            reconstructed[-1] = reconstructed[-1].rstrip() + buf.lstrip()
            continue

        # enquanto o buffer tiver aspas desbalanceadas OU a próxima linha começar com ';',
        # juntamos as partes (preservando newlines dentro de campos quando necessário)
        while True:
            if i >= n:
                break
            next_ln = clean_lines[i]
            next_lstr = next_ln.lstrip()

            # se próximo é continuação com ';', anexar sem inserir nova linha
            if next_lstr.startswith(';'):
                buf = buf.rstrip() + next_lstr
                i += 1
                continue

            # se buf tem aspas não fechadas, juntamos com newline (mantendo o newline dentro do campo)
            if not is_balanced_quotes(buf):
                buf = buf + "\n" + next_ln
                i += 1
                continue

            # se buf está balanceado e próximo não começa com ';' -> é nova linha
            break
    ...
    # ---- 3) Tenta ler o CSV reconstruído ----
    try:
        df_corr = pd.read_csv(
        ...
        try:
            window["OUTPUT"].print(f"⚠ Falha ao ler CSV reconstruído: {e}")
            window.refresh()
        except Exception:
            pass
        return False
    ...
    # ---- 4) Aplica a correção no `df` original (mesma lógica usada antes) ----
    try:
        ...
        try:
            window["OUTPUT"].print(f"⚠ Falha ao aplicar DataFrame reconstruído: {e}")
            window.refresh()
        except Exception:
            pass
        return False
...
# =========================
layout = [
    [sg.Text("Arquivo de códigos (.xlsx ou .csv):"),
     sg.Input(key="-ARQUIVO-"),
     sg.FileBrowse(button_text='Procurar', file_types=(("Excel Files", "*.xlsx"), ("CSV Files", "*.csv")))],
    [sg.Text("Pasta para arquivos corrompidos:"),
     sg.Input(key="-PASTA-"),
     sg.FolderBrowse(button_text='Procurar')],
    [sg.Text("Tamanho da página:"), sg.Input("500", key="-TAMANHO-", size=(6,1)),
     sg.Text("Timeout (segundos):"), sg.Input("60", key="-TIMEOUT-", size=(6,1))],
...
while True:
    ...
    if event == "Iniciar":
        ...
        TAMANHO_PAGINA = int(values["-TAMANHO-"])
