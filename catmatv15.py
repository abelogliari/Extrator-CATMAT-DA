...
def pagina_corrompida(csv_text: str, INDICE_COLUNA_T: int, INDICE_COLUNA_AI: int) -> Tuple[bool, Optional[pd.DataFrame], str]:
    """
    Detecta e tenta corrigir CSV corrompido.

    Retorna (is_corrupt, df_corrigido_or_None, csv_corrigido_text).

    - is_corrupt: True se detectou anomalias (linhas quebradas/colunas removidas).
    - df_corrigido_or_None: DataFrame reconstituído se possível, senão None.
    - csv_corrigido_text: o CSV reconstruído (mesmo que a leitura falhe).
    """
    if not csv_text:
        return False, None, csv_text
    ...
    while i < len(linhas):
        ...
        if linha_raw.strip().lower().startswith("totalregistros:") or linha_raw.strip().lower().startswith("totalpaginas:"):
            i += 1
            continue

        lstripped = linha_raw.lstrip()

        # Caso A: linha termina com aspas desbalanceada -> junta com a próxima linha inteira
        if linha_raw.rstrip().endswith('"') and (linha_raw.count('"') % 2 == 1):
            if i + 1 < len(linhas):
                linha_raw = linha_raw + linhas[i + 1].lstrip()
                cont_corrections = True
                i += 2
                # agora processar linha_raw normalmente abaixo
            else:
                i += 1
                continue
        else:
            # Caso B: a linha atual é uma aspa isolada -> anexar o começo da próxima linha ao último campo da linha anterior
            ...
            # Caso C: linha começa com aspas + conteúdo -> anexar esse conteúdo ao último campo da linha anterior
            ...
            # Caso normal: processa a linha sem joins
            i += 1

        partes = linha_raw.split(";")

        # Corrige colunas problemáticas (intervalo T..AI)
        max_j = min(len(partes) - 1, INDICE_COLUNA_AI)
        for j in range(INDICE_COLUNA_T, max_j + 1):
            cell = (partes[j] or "")

            # regras aceitáveis (pular)
            if re.search(r'""', cell):
                continue
            if re.match(r"^'.*'$", cell):
                continue
    ...
    # Se não detectou nenhuma correção, retorna que não está corrompido
    if not (cont_corrections or col_corrections):
        return False, None, csv_text

    # Reconstrói CSV corrigido
    ...
    try:
        ...
        print(f"[pagina_corrompida] Erro ao reler CSV corrigido: {e}")
        return True, None, csv_corrigido
...
while True:
    ...
    if event == "Iniciar":
        ...
        for idx, codigo in enumerate(codigos_itens, 1):
            ...
            while True:
                try:
                    # ---> Chamada única para detectar e tentar reconstruir
                    is_corrupt, df_corrigido, csv_corrigido = pagina_corrompida(csv_text, INDICE_COLUNA_T, INDICE_COLUNA_AI)
                    ...
                    if is_corrupt:
                        ...
                        with open(arquivo_corrompido, "w", encoding="utf-8-sig", errors="replace") as f:
                            # se dfp possui colunas, grava cabeçalho; senão, tenta extrair do csv_corrigido
                            try:
                        ...
                        if df_corrigido is not None and not df_corrigido.empty:
                            ...
                            window["OUTPUT"].print(f"⚠ Página {pagina_atual} do código {codigo} marcada como corrompida, mas não foi possível reconstruí-la.")
                            window.refresh()

                    else:
                        # página aparentemente OK: usa dfp (parsing original)
                        if not dfp.empty:
        ...
        for codigo in codigos_itens:
            ...
            if "Descartado (0 registros)" in paginas:
                ...
                registros_esperados_api = registros_validos

            relatorio_dados.append({
                "codigoItemCatalogo": codigo,
                "registros_esperados": registros_esperados_api,
